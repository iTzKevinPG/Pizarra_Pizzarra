...
function canvasApp() {
	...
	function init(){
		...
		function startLine(e, selectedColor){
			context.beginPath();
			context.strokeStyle = selectedColor; // Utiliza el color seleccionado
		...
		socket.on('connect', function(){
			...
			theCanvas.addEventListener("mousedown", function (e) {
				if (!block) {
					...
					socket.emit('startLine', { clientX: e.clientX, clientY: e.clientY, color: selectedColor });
					click = true;
					startLine(e, selectedColor);
				}
			}, false);
